<div class="row js-form-project-detail m-b-30 {{!$errors->any() ? '' : 'd-none'}}">
    @if($project?->getCostBenefit(false) != null)
        ...
        <div class="col-md-12 m-l-10 m-t-30">
            <label>Attachment File : </label>
            @if(isset($project->cost_benefits?->attachment))
                <a target="_blank" href="/preview?dir={{urlencode($project->project_name)}}&category=Cost Benefit&file={{urlencode($project->cost_benefits?->attachment)}}">
                    <i class="fa mb-2 fa-file-text-o txt-info"></i>
                    {{$project->cost_benefits?->attachment}}
                </a>
            @endif
        </div>
    @else
